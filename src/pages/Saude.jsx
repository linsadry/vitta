...
// ─── SHARED: small pill tag ────────────────────────────────────────
function Tag({children, color}) {
  return (
    <span style={{
      display:'inline-flex',alignItems:'center',padding:'3px 9px',borderRadius:'var(--r-full)',
      background:(color||'var(--c-text-300)')+'1E',color:color||'var(--c-text-500)',
      fontFamily:'var(--font-ui)',fontSize:11,fontWeight:500,
    }}>{children}</span>
  )
}

// ─── SHARED: row action buttons (check / edit / delete) ───────────
function RowActions({done, onToggleDone, onEdit, onDelete}) {
  const btn = (extra) => ({
  ...
  return (
    <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
      <button onClick={onToggleDone} style={btn({background:done?'var(--c-sage)':'var(--c-base-2)',color:done?'#fff':'var(--c-text-300)'})} title={done?'Marcado como feito':'Marcar como feito'}>
        <Check size={14} strokeWidth={2.5}/>
      </button>
      <button onClick={onEdit} style={btn({background:'var(--c-base-2)',color:'var(--c-text-500)'})} title="Editar">
        <Pencil size={13} strokeWidth={1.8}/>
      </button>
      <button onClick={onDelete} style={btn({background:'rgba(212,165,165,0.15)',color:'var(--c-rose-mid)'})} title="Excluir">
  ...
  return (
    <div className="sheet-overlay" onClick={onClose}>
      <div className="sheet" onClick={e=>e.stopPropagation()}>
        ...
        <div style={{marginBottom:20}}><label className="input-label">Orientações / resultado</label><textarea className="input-field" rows={3} value={f.notes} onChange={e=>set('notes',e.target.value)} style={{resize:'none'}}/></div>
        <button className="btn-primary" onClick={save} disabled={saving||!f.date||!f.specialty}>{saving?'Salvando...':isNew?'Salvar consulta':'Salvar alterações'}</button>
      </div>
    </div>
  )
}
...
// ─── EXAM MODAL ───────────────────────────────────────────────────
function ExamModal({item, userId, onClose, onSave}) {
  const isNew = !item?.id
  const [f,setF]=useState({
  ...
  const save=async()=>{
    ...
    const { error } = isNew
      ? await supabase.from('lab_results').insert({ user_id: userId, ...payload })
      : await supabase.from('lab_results').update(payload).eq('id', item.id)
  ...
  return (
    <div className="sheet-overlay" onClick={onClose}>
      <div className="sheet" onClick={e=>e.stopPropagation()}>
        <div className="sheet-handle"/>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
          <h2 className="sheet-title" style={{marginBottom:0}}>{isNew?'Novo exame':'Editar exame'}</h2>
          <button onClick={onClose} className="btn-ghost" style={{padding:8}}><X size={18} strokeWidth={1.8}/></button>
        </div>
        ...
        <button className="btn-primary" onClick={save} disabled={saving||!f.category}>{saving?'Salvando...':isNew?'Salvar':'Salvar alterações'}</button>
      </div>
    </div>
  )
}
...
function ConsultasView({userId, consultas, onBack, onReload}) {
  ...
  const toggleAttended = async (c) => {
    await supabase.from('health_consultations').update({ attended: !c.attended }).eq('id', c.id)
    onReload()
  }
  const deleteItem = async (c) => {
    if (!window.confirm(`Excluir a consulta "${c.specialty||'Consulta'}"?`)) return
  ...
  const CRow = ({c}) => (
    <div style={{padding:'14px 0',borderBottom:'1px solid var(--c-border-light)'}}>
      <div style={{display:'flex',alignItems:'flex-start',gap:14}}>
        <div style={{width:34,height:34,borderRadius:9,background:'var(--c-rose)18',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:2}}>
          <Stethoscope size={15} strokeWidth={1.8} style={{color:'var(--c-rose)'}}/>
        </div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:10}}>
            <div style={{fontFamily:'var(--font-ui)',fontSize:14,fontWeight:500,color:'var(--c-text-900)'}}>{c.specialty||'Consulta'}{c.doctor?` · ${c.doctor}`:''}</div>
            <RowActions done={!!c.attended} onToggleDone={()=>toggleAttended(c)} onEdit={()=>setSelected(c)} onDelete={()=>deleteItem(c)}/>
          </div>
          <div style={{display:'flex',flexWrap:'wrap',gap:6,marginTop:6}}>
            <Tag color={c.attended?'var(--c-sage)':'var(--c-gold)'}>{c.attended?'Realizada':'Agendada'}</Tag>
            {c.location && <Tag color="var(--c-text-400)"><MapPin size={10} strokeWidth={2} style={{marginRight:3}}/>{c.location}</Tag>}
          </div>
          <div style={{fontSize:12,color:'var(--c-text-500)',marginTop:6}}>{formatDate(c.date)}{c.time&&` · ${c.time}`}</div>
          {(c.diagnosis||c.notes) && (
            <p style={{fontFamily:'var(--font-editorial)',fontSize:13,color:'var(--c-text-600)',fontStyle:'italic',lineHeight:1.5,marginTop:6,marginBottom:0}}>
              {[c.diagnosis,c.notes].filter(Boolean).join(' — ')}
            </p>
          )}
        </div>
      </div>
        ...
        {months.map(mk=>(
          <div key={mk} style={{marginBottom:24}}>
            ...
            <div className="card" style={{padding:'0 16px'}}>
              {byMonth[mk].map(c=><CRow key={c.id} c={c}/>)}
            </div>
          </div>
        ))}
      </div>
...
function ExamesView({userId, labResults, onBack, onReload}) {
  ...
  const STATUS_LABEL = { pendente:'Pendente', agendado:'Agendado', realizado:'Realizado' }
  const STATUS_COLOR = { pendente:'var(--c-text-400)', agendado:'var(--c-gold)', realizado:'var(--c-sage)' }
  ...
  const toggleDone = async (e) => {
    const willBeDone = e.status !== 'realizado'
    const payload = { status: willBeDone ? 'realizado' : 'pendente' }
    if (willBeDone && !e.date) payload.date = today()
    await supabase.from('lab_results').update(payload).eq('id', e.id)
    onReload()
  }
  const deleteItem = async (e) => {
    if (!window.confirm(`Excluir o exame "${e.category}"?`)) return
    await supabase.from('lab_results').delete().eq('id', e.id)
    onReload()
  }
  ...
  const ExRow=({e})=>(
    <div style={{padding:'14px 0',borderBottom:'1px solid var(--c-border-light)'}}>
      <div style={{display:'flex',alignItems:'flex-start',gap:14}}>
        <div style={{width:34,height:34,borderRadius:9,background:'var(--c-sage-faint)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:2}}>
          <FlaskConical size={15} strokeWidth={1.8} style={{color:'var(--c-sage)'}}/>
        </div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:10}}>
            <div style={{fontFamily:'var(--font-ui)',fontSize:14,fontWeight:500,color:'var(--c-text-900)'}}>{e.category}</div>
            <RowActions done={e.status==='realizado'} onToggleDone={()=>toggleDone(e)} onEdit={()=>setSelected(e)} onDelete={()=>deleteItem(e)}/>
          </div>
          <div style={{display:'flex',flexWrap:'wrap',gap:6,marginTop:6}}>
            <Tag color={STATUS_COLOR[e.status]||'var(--c-text-400)'}>{STATUS_LABEL[e.status]||'Realizado'}</Tag>
            {e.location && <Tag color="var(--c-text-400)"><MapPin size={10} strokeWidth={2} style={{marginRight:3}}/>{e.location}</Tag>}
          </div>
          <div style={{fontSize:12,color:'var(--c-text-500)',marginTop:6}}>{formatDate(e.scheduled_date||e.date)}</div>
          {e.notes && (
            <p style={{fontFamily:'var(--font-editorial)',fontSize:13,color:'var(--c-text-600)',fontStyle:'italic',lineHeight:1.5,marginTop:6,marginBottom:0}}>
              {e.notes}
            </p>
  ...
  return (
    <div>
      ...
      {selected&&<ExamModal item={selected} userId={userId} onClose={()=>setSelected(null)} onSave={()=>{setSelected(null);onReload()}}/>}
    </div>
  )
}
