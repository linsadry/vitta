...
function useHomeData(userId) {
  ...
  const load = useCallback(async () => {
    ...
    const [
      ...
      {data:cycleEntries},{data:upConsults},
    ] = await Promise.all([
      ...
      supabase.from('lab_results').select('category,scheduled_date').eq('user_id',userId).eq('status','agendado').order('scheduled_date').limit(1).maybeSingle(),
